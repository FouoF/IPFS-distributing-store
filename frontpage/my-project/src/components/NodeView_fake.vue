  ...
  export default {
    ...
    data() {
      return {
        nodes: [],
        dialogVisible: false,
        newNode: { node: { id: '', name: '', addr: '', status: '未知' } },
      };
    },
    methods: {
      openAddNodeDialog() {
      ...
      addNewNode() {
        // 模拟添加节点
        const newNode = { ...this.newNode.node, status: '创建中' };
        this.nodes.push(newNode);
  
        // 模拟状态变化
        setTimeout(() => {
          newNode.status = '运行中';
          this.$message.success('Node added successfully');
      ...
      resetForm() {
        this.newNode = { node: { id: '', name: '', addr: '', status: '未知' } };
      },
    },
  };
  </script>
